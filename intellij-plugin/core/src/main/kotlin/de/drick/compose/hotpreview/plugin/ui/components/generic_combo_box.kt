...
fun <T: Any>GenericComboBox(
    ...
    val currentSelectedItem = remember(selectedItem) {
    ...
    LaunchedEffect(currentSelectedItem) {
        scrollState.selectedKeys = items.getOrNull(currentSelectedItem)?.let { setOf(it) } ?: emptySet()
    }
    ...
    val onArrowDownPress: () -> Unit = {
        ...
        selectItem(currentSelectedItem.plus(1).coerceAtMost(items.lastIndex))
        scope.launch { scrollState.lazyListState.scrollToIndex(currentSelectedItem) }
    }
    val onArrowUpPress: () -> Unit = {
        ...
        selectItem(currentSelectedItem.minus(1).coerceAtLeast(0))
        scope.launch { scrollState.lazyListState.scrollToIndex(currentSelectedItem) }
    }
        ...
        VerticallyScrollableContainer(
            ...
            SelectableLazyColumn(
                ...
                onSelectedIndexesChange = { selectedItemsIndexes ->
                    ...
                    if (selectedItemsIndexes.first() == currentSelectedItem) return@SelectableLazyColumn
